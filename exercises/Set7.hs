...
add val set = if member val set
              then set
              else Set (insert val xs)
                where (Set xs) = set 
...
average (x :| []) = x
average (x :| xs) = sum xs / fromIntegral (length xs + 1)
  where sum = foldl (+) x
...
reverseNonEmpty (x :| xs) = foldl (\(y :| ys) z -> z :| (y : ys)) (x :| []) xs
...
passwordAllowed pwd (ContainsSome s) = any (`elem` pwd) s
passwordAllowed pwd (DoesNotContain s) = all (`notElem` pwd) s
...
render x = case x of
  Literal i -> show i
  Operation op a b -> "(" ++ render a ++ op ++ render b ++ ")"
