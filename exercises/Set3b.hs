...
sums :: Int -> [Int]
sums i =
    let sumsHelper :: Int -> Int -> [Int]
        sumsHelper current max =
            if current > max
            then []
            else div (current * (1 + current)) 2 : sumsHelper (current + 1) max
    in  sumsHelper 1 i
...
sumsOf (x:xs) =
    let sumsHelper :: Int -> [Int] -> [Int]
        sumsHelper acc [] = []
        sumsHelper acc (y:ys) = (acc + y) : sumsHelper (acc + y) ys
    in  x : sumsHelper x xs
