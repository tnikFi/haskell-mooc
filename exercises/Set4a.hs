...
longest xs = minimum $ filter (\x -> length x >= length (last (sortBy (comparing length) xs))) xs
...
incrementKey k [] = []
incrementKey k ((x,y):xs) = if x == k then (x,y+1) : incrementKey k xs else (x,y) : incrementKey k xs
